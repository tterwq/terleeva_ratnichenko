...
public class SoftStopTest
{
    ...
    public void SoftStopCommandStrategyWithActionTests()
    {
        ...
        IoC.Resolve<ICommand>("Send Command", "5", SSCmd).Execute();
    ...
    public void SoftStopCommandStrategyWithoutActionTests()
    {
        ...
        IoC.Resolve<ICommand>("Send Command", "6", new ServerThreadDependecies(dictThread, dictReceiver, dictSender)).Execute();

        var SSCmd = IoC.Resolve<ICommand>("SoftStopThread", "6");
        IoC.Resolve<ICommand>("Send Command", "6", SSCmd).Execute();
        IoC.Resolve<ICommand>("Send Command", "6", cmd1.Object).Execute();

        mre.WaitOne();
        cmd1.Verify(c => c.Execute(), Times.Once());

        var receiver = IoC.Resolve<IReceiver>("Get Receiver", "6");
        Assert.True(receiver.isEmpty());
    }

    [Fact]
    public void SoftStopThrowsExceptionTest()
    {
        ...
        var softStopCmd = new SoftStopCommand("8", IoC.Resolve<MyThread>("Get Thread", "8"));
